...
include("dados_comentario_pagina_bd.php");
...
while ($contador1<$contador) { 
  ...
  if($contador1<3){
	...
	if($emailusuario==$email[$contador1]){
	  echo "<td><label>&nbsp<a href='menu?p=$codusuario1[$contador1]'>$nomeusario1[$contador1]</a></label>&nbsp&nbsp<font size=1 color=black>$hora[$contador1] - $data[$contador1]</font><a href=menu?expagina=$id[$contador1]><img src='imagem/lixeira.png' width='23px'></a></td></tr> </table></hr>";
	}
	else {
	...
	echo "
	<br><label><p align='justify'>$comentario[$contador1]  </hr></p></label><br>
	</div>";
	}
...
$contador1++;
	...
	if($contador1>3){
      echo "<div class='white-panelcomentario1  pncomentario1'>
     <div class='col-md-12'> 
	<br><a href='menu?publi=$id_banco[$cont]'><input type='button' class='btn btn-success btn-block' value='Ver mais'></a>
	</div>
	</div>";
	}
?>